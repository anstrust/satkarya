...
<div class="form-box" style="width:95%; margin:30px; "  id="template-box">
    ...
    <form  action="<?php echo HTTP_ROOT?>admin/users/edit_career" method="POST" id="editTemplate" enctype="multipart/form-data">
    <input type="hidden" class="required" name="data[Job][id]" value='<?php if(!empty($info['Job']['id'])){ echo $info['Job']['id']; } ?>' />           
        <div class="body bg-gray">
            <div class="form-group title_div">
              Title:<input type="text" class="required form-control cms_title"  name="data[Job][title]" value='<?php if(!empty($info['Job']['title'])){ echo $info['Job']['title']; } ?>' />
            </div>
            <div id='title'></div>
            <div class="form-group">
               Description:<textarea id='description' name="data[Job][description]" rows="45" cols="44" class="required form-control ckeditor"><?php if(!empty($info['Job']['description'])){ echo $info['Job']['description']; } ?></textarea>
            </div>
            <div id='des_id'></div>
